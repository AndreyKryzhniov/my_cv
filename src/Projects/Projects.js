...
function Projects() {
    ...
    let informationTodoList = () => {
        return (
            <lo>
                ...
                <li>Сортировка отоброжения выполненных/невыполненных задач</li>
            </lo>
        )
    }
    ...
    let informationRandomChat = () => {
        return (
            <lo>
                <li>По нажатию единственной кнопки вас соеденяет со случайным пользователем</li>
                <li>Возможность отправки сообщений</li>
                <li>Возможность Log in/Log out</li>
                <li>В данный момент проект в разработке (для тестирования смотереть Network)</li>
            </lo>
        )
    }
